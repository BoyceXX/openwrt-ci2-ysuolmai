...
# [修复 mbedtls GCC14 编译报错] 强制关闭 ARM 汇编优化
echo "Patching mbedtls for GCC 14..."
# 1. 强制加入 -fno-builtin-memset 防止内联函数报错
find package/ -name "Makefile" | xargs grep -l "mbedtls" | xargs sed -i 's/$(TARGET_CFLAGS)/$(TARGET_CFLAGS) -fno-builtin-memset/g'
# 2. 强制关闭 ARMv8 硬件加速 (双重保险)
...
# [修复 uhttpd 依赖] 强制 uhttpd 使用 openssl 而非 mbedtls
...
UPDATE_PACKAGE "xray-core xray-plugin dns2tcp dns2socks haproxy hysteria \
        ...
        netdata luci-app-netdata lucky luci-app-lucky luci-app-openclash mihomo \
...
keywords_to_delete=(
    ...
    "ddns" "LSUSB" "mihomo" "smartdns" "kucat" "bootstrap"
    "mbedtls" # 强制清理 config 中的 mbedtls，防止自动勾选
)
...
provided_config_lines=(
    ...
    "CONFIG_PACKAGE_luci-i18n-ddns-go-zh-cn=n"
...
if [[ $FIRMWARE_TAG == *"NOWIFI"* ]]; then
    ...
    # 删除 Makefile 中的驱动编译，防止生成 kmod
    if [[ "$FIRMWARE_TAG" != *"EMMC"* && "$FIRMWARE_TAG" != *"IPQ807X"* ]]; then
...
rm -f package/kernel/mac80211/patches/nss/subsys/{999-775-wifi-mac80211-Changes-for-WDS-MLD.patch,999-922-mac80211-fix-null-chanctx-warning-for-NSS-dynamic-VLAN.patch}
